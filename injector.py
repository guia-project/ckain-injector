...
def dataset_name_from_row(row: Dict[str, str]) -> str:
    ...
    study_plan_code = row.get("study_plan_code", "")
    subject_code = row.get("subject_code", "")
    subject_name = row.get("subject_name", "")

    raw = f"{academic_year}-{semester}-{study_plan_code}-{subject_code}-{subject_name}"
...
def dataset_title_from_row(row: Dict[str, str]) -> str:
    ...
    return f"{subject_name} [{subject_code}] - {year} {semester}"
...
def build_notes(row: Dict[str, str]) -> str:
    ...
    extra_pages = [
        ("Datos descriptivos", row.get("info_description_url", "")),
        ("Profesorado", row.get("info_professors_url", "")),
        ("Requisitos previos", row.get("info_prev_requirements_url", "")),
        ("Competencias y resultados", row.get("info_competences_and_results_url", "")),
        ("Temario", row.get("info_syllabus_url", "")),
        ("Cronograma", row.get("info_schedule_url", "")),
        ("Evaluación", row.get("info_evaluation_url", "")),
        ("Recursos didácticos", row.get("info_resources_url", "")),
        ("Otra información", row.get("info_other_url", "")),
    ]

    lines.append("")
    lines.append("Páginas relacionadas:")
    for label, url in extra_pages:
        if url:
...
def ensure_dataset(
    ...
    try:
        ...
        print(f"[OK] Dataset actualizado: {dataset_name}")
        return updated

    except NotFound:
        created = ckan.action.package_create(**payload)
        print(f"[OK] Dataset creado: {dataset_name}")
        return created
...
def ensure_pdf_resource(
    ...
    if not pdf_url:
        print(f"[WARN] Fila sin guide_pdf_url, se omite recurso: {dataset.get('name')}")
        return None

    current = ckan.action.package_show(id=dataset["id"])

    if resource_already_exists(current.get("resources", []), pdf_url):
        print(f"[SKIP] Recurso ya existe para {dataset.get('name')}: {pdf_url}")
        return None

    resource_name = f"Guía docente PDF - {row.get('subject_name', '').strip()}"
    description = (
        f"PDF remoto de la guía docente de {row.get('subject_name', '').strip()} "
        f"({row.get('academic_year', '').strip()} {row.get('semester', '').strip()})"
    )
    ...
    print(f"[OK] Recurso PDF creado en {dataset.get('name')}")
    return resource
    ...
    with open(csv_path, "r", encoding="utf-8", newline="") as f:
        ...
        for idx, row in enumerate(reader, start=1):
            ...
            try:
                ...
                ensure_pdf_resource(ckan=ckan, dataset=dataset, row=row)

            except CKANAPIError as e:
                print(f"[ERROR] Fila {idx} ({row.get('subject_code')}): {e}", file=sys.stderr)
